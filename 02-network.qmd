...
You can download a subset of the POI dataset via the link provided below. A copy of the 2021 London LSOAs spatial boundaries, the boundaries of the London Boroughs, and the 2019 English [Index of Multiple Deprivation](https://www.gov.uk/government/statistics/english-indices-of-deprivation-2019). Save these files in your project folder under `data`.
...
#| fig-cap: Extracted road network data for Lambeth.
#| classes: styled-output
#| echo: True
#| eval: True
#| filename: "R code"
...
This looks good. Since our focus is on schoolchildren, we will overwrite the `oneway` variable to assume that none of the road segments are restricted to one-way traffic. This adjustment will ensure our analysis is not skewed by such restrictions and will help maintain a more accurate representation of the general connectivity of the network.
...
### Network preparation
In any network analysis, the primary data structure is a graph composed of nodes and edges. The `dodgr` library utilises weighting profiles to assign weights based on road types, tailored to the mode of transport that each profile is designed to model.

In this instance, we will use the `foot` weighting profile, as our focus is on modelling walking accessibility. To prevent errors related to the weighting profile, we will replace any `NA` values in the highway tag with the value `unclassified`.
...
osm_network_graph_connected <- osm_network_graph[osm_network_graph$component == 1, ]

# inspect number of remaining road segments
nrow(osm_network_graph)
```
...
distance_matrix <- dodgr_distances(
  osm_network_graph_connected,
  from=st_coordinates(poi_schools),                                to= st_coordinates(poi_fastfood), 
  shortest = FALSE, 
  pairwise = FALSE, 
  quiet=FALSE)
```
...
To do:
- check input data in QGIS for NA distances
- check shortest = FALSE to see if travel time works
